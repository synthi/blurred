...
local function draw_lfo_icon(shape, x, y)
  screen.level(15); local w=24; local h=10; local x_start=x-(w/2); local y_mid=y+6; local y_top=y_mid-5; local y_bot=y_mid+5
  if shape == 1 then screen.move(x_start,y_mid); for i=0,w do screen.pixel(x_start+i, y_mid+math.sin((i/w)*12.56)*5) end; screen.stroke()
  elseif shape == 2 then screen.move(x_start,y_mid); screen.line(x_start+w/4,y_top); screen.line(x_start+w/2,y_mid); screen.line(x_start+(3*w)/4,y_bot); screen.line(x_start+w,y_mid); screen.stroke()
  elseif shape == 3 then screen.move(x_start,y_bot); screen.line(x_start+w/2,y_top); screen.line(x_start+w/2,y_bot); screen.line(x_start+w,y_top); screen.line(x_start+w,y_bot); screen.stroke()
  elseif shape == 4 then local c=w/2; screen.move(x_start,y_top); screen.line(x_start+c/2,y_top); screen.line(x_start+c/2,y_bot); screen.line(x_start+c,y_bot); screen.line(x_start+c,y_top); screen.line(x_start+c*1.5,y_top); screen.line(x_start+c*1.5,y_bot); screen.line(x_start+w,y_bot); screen.stroke()
  elseif shape == 5 then screen.move(x_start,y_mid); for i=0,w,4 do local r=math.random(-4,4); screen.line(x_start+i,y_mid+r); screen.line(x_start+i+4,y_mid+r) end; screen.stroke()
  elseif shape == 6 then for i=0,w do screen.pixel(x_start+i,y_mid+math.random(-3,3)) end; screen.fill() end
end

local function draw_plasma_drop(x, y, size)
  screen.level(4); screen.circle(x, y, size + 2.5); screen.fill()
  screen.level(15); screen.circle(x, y, size + 1.0); screen.fill()
  screen.level(15); screen.pixel(x + 2, y - 2); screen.fill()
end

local function draw_spray_point(x, y, radius, density, sharpness)
  if sharpness > 0.8 then screen.pixel(x, y); screen.fill()
  else
     local effective_rad = radius * (1.5 - sharpness)
     for i = 1, density do
       local r = math.random() * effective_rad; local a = math.random() * 6.28
       screen.pixel(x + math.cos(a)*r, y + math.sin(a)*r); screen.fill()
     end
  end
end

local function draw_chain(state)
  local cx=61; local cy=34; local stages=50 
  local time_scale=params:get("time_scale"); local freq=params:get("frequency"); local wander=params:get("wander")*0.4; local skew=params:get("skew")*0.5     
  local div_base=params:get("div_base")*0.55; local lfo_rate=params:get("lfo_rate"); local lfo_amt=params:get("lfo_amt")
  local feedback=params:get("feedback"); local grit=params:get("grit"); local decay=params:get("decay"); local depth=params:get("depth")
  local polarity=params:get("polarity"); local damping=params:get("damping") 
  
  local sharpness=util.linlin(0,2,0,1,decay); if sharpness>1 then sharpness=1 end
  ...
  for i=1,stages do
    ...
    local norm_x=(i-1)/(stages-1); local centered_x=(norm_x-0.5)*2; local parabola=centered_x*centered_x; local poly_offset=parabola*polarity*-12 
    local grit_jit_x=(math.random()-0.5)*jitter_amt; local grit_jit_y=(math.random()-0.5)*jitter_amt
    local px=start_x+(i*step)+grit_jit_x; local py_base=cy+poly_offset+(math.sin((i*freq*0.5)+anim_phase)*wander*15)+grit_jit_y
  ...
  for t=#trail_history,1,-1 do
    ...
    if brightness>0 then screen.level(brightness)
        for _,pt in ipairs(frame) do
          if brightness>1 or math.random()>0.6 then draw_spray_point(pt.x, pt.y_l, pt.rad, pt.den, pt.sharp); draw_spray_point(pt.x, pt.y_r, pt.rad, pt.den, pt.sharp) end
          if t==1 then if pt.ball_l then draw_plasma_drop(pt.x, pt.y_l, 2.0) end; if pt.ball_r then draw_plasma_drop(pt.x, pt.y_r, 2.0) end end
        end
    end
  end
end
...
function Graphics.draw(state)
  ...
  if state.current_page == state.PAGES.SYNTH then
    if not shift_active then
        ...
        draw_param_inline("Attack", util.round(params:get("vintage_attack"), 0.01), 0, bot_y, "left")
        draw_param_inline("Decay", util.round(params:get("vintage_decay"), 0.01), 128, bot_y, "right")
    end
